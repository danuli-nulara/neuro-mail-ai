...
def predict(email_text):
    ...
    try:
        ...
        prompt = f"""You are an email security expert. This email was classified as {label} by an AI model.

Email content:
{email_text}

Give exactly 3 short bullet point reasons WHY this email is {label}.
Each reason should be one line max.
...
- [reason 3]"""
    ...
    return {
        ...
        "Email threat analysis": explanation
    }
        ...
        with gr.Tab("✍️ Email Generator"):
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    def copy_email(text):
                        if not text:
                            return "<p style='color:#e24b4a; font-size:0.85rem;'>⚠️ No email to copy yet!</p>"
                        return f"""
                            <script>navigator.clipboard.writeText({repr(text)});</script>
                            <p style='color:#1d9e75; font-size:0.85rem;'>✅ Copied to clipboard!</p>
                        """

                    generate_btn.click(fn=generate_email, inputs=[topic_input, tone_input], outputs=email_output)
                    copy_btn.click(fn=copy_email, inputs=email_output, outputs=copy_status)
